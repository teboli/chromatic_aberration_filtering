...
def ti_and_ca_filtering1D(X_in, G_in, Y_in, L, rho, alpha_X, tau):
    ...
    K_TI_hor = np.zeros_like(X_in)
    K_hor = np.zeros_like(X_in)
    X_max = np.zeros_like(X_in)
    X_min = np.zeros_like(X_in)
    M, N = X_in.shape
...
def arbitration(K, K_TI, X, G, X_max, X_min, beta_X, L_hor, L_ver, gamma_1=0.5, gamma_2=0.25):
    ...
    X_contrast_hor = np.zeros_like(X)  # (M, N), horizontal pass
    for j in range(N):
    ...
    X_contrast_ver = np.zeros_like(X.T)  # (N, M), vertical pass
    for j in range(M):
